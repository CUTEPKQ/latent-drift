...
class Transformer(nn.Module):
    ...
    def _layer_forward(self, peg, self_attn, cross_attn, ff, x, video_shape, attn_bias, context, self_attn_mask, cross_attn_context_mask):
        if exists(peg):
    ...
    def forward(
        ...
        for peg, self_attn, cross_attn, ff in self.layers:
            if self.grad_checkpointing and self.training:
                x = checkpoint(self._layer_forward, peg, self_attn, cross_attn, ff, x, video_shape, attn_bias, context, self_attn_mask, cross_attn_context_mask, use_reentrant=False)
            else:
                x = self._layer_forward(peg, self_attn, cross_attn, ff, x, video_shape, attn_bias, context, self_attn_mask, cross_attn_context_mask)
